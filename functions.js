...
//bu fonksiyon yardımıyla tarayıcının cache belleğine yaptığımız işlemleri
//kaydediyoruz veya siliyoruz 
simcir.$ = function() {
  ...
  var cacheIdSeq = 0;
  var cache = {};
//cachedeki veriyi almak için getCache fonksiyonu
...
//cachede aradığımız verinin olup olmadığını kontrol eden hasCache fonksiyonu
...
//cachedeki veriyi silen fonksiyon
  var removeCache = function(elm) {

    if (typeof elm[cacheIdKey] != 'undefined') {

      // işlem olup olmadığını kontrol eden listenerları silen fonksiyon
      var cacheId = elm[cacheIdKey];
	...
	//tüm veriler boşalana kadar cachedeki verileri silen fonksiyon
...
//listenerları çeken fonksiyon
  ...
  // event listener ekleyen fonksiyonlar
...
//hashmapte tutulan verileri kontrol eden fonksiyon 
...
//iki diziyi karşılaştıran fonksiyon
  var extend = function(o1, o2) {
    for (var k in o2) { o1[k] = o2[k]; } return o1;
  };
//verinin fonksiyon olup olmadığını kontrol eden fonksiyon
...
//listedeki verileri kontrol edip doğru formda ise yeni listeye atan fonskiyon
...
//yeni bir class ekleyen fonksiyon
...
//classın var olup olmadığını kontrol eden fonksiyon
...
//selectorlerin eşleşip eşleşmediğini kontrol eden fonksiyon
...
//html dosyası oluşturan bir fonksiyon
...
//sorgu oluşturan fonksiyon
  ...
  var parseResponse = function() {
	...
	//html ile javascript arasında veri alışverisini şağlayan fonksiyon
  ...
  // 1. tek eleman için verileri işleyen fonksiyon
  var fn = {
    attr : function(kv) {
  ...
  // birden fazla elemanı fn fonksiyonu ile kontol edecek fonksiyon
  ...
  // 3. Array için verileri işleyen fonksiyon
  fn = extend(fn, {
...
// çizimlerin yapıldığı fonksiyonlar
!function($s) {
...
//bağlantıların yapıldığı fonksiyon
...
//event kuyruğunu oluşturan fonksiyon
  ...
  var unit = 16;
  var fontSize = 12;
//label oluşturan fonksiyon
  var createLabel = function(text) {
    return createSVGElement('text').
      text(text).
      css('font-size', fontSize + 'px');
  };
	//input ve output nodelarını oluşturan fonksiyon
...
//nodun aktif olup olmadığını kontrol eden fonksiyon
  var isActiveNode = function($o) {
    return $o.closest('.simcir-node').length == 1 &&
      $o.closest('.simcir-toolbox').length == 0;
  };
//node kontrol etmek için controller oluhşturan fonksiyon
...
//input nodelarını kontrol edecek fonksiyouun oluşturulması
...
//output nodelarını kontrol edecek fonksiyouun oluşturulması
...
//kapıları oluşturan fonksiyon
  var createDevice = function(deviceDef, headless, scope) {
	...
	//kapıları kontrol edecek controllerı oluşturan fonksiyon
  var createDeviceController = function(device) {
    ...
    var getSize = function() {
      ...
      return { width: unit * 2.5,
        height: unit * Math.max(3.5, device.halfPitch?
            (nodes + 1) / 2 : nodes)};
    };
...
//devreyi oluşturan fonksiyon
//cacheden gelen verileri işleyerek devreyi oluşturan fonksiyon
...
//boarda kapıları ekleyen silen yönetim fonksiyonu
...
//pencereyi şekillendiren fonksiyonlar
...
//toolbox oluşturan fonksiyonlar
  ...
  var factories = {};
  var defaultToolbox = [];
  //toolboxda bulunacak kapıları ekleyen fonksiyon
...
//bağlantıları kaldıran fonksiyon
...
!function($s) {
  ...
  // Nodeları oluşturan fonksiyonların çağırılması
